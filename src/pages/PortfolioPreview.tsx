...
export default function PortfolioPreview() {
  ...
  const handleExportPdf = async () => {
    ...
    pdf.save(`${portfolio.title.toLowerCase().replace(/\s+/g, '-')}-portfolio.pdf`);
  };
      ...
      <div id="portfolio-content">
                  ...
                  return (
                    ...
                    >
                      <div className="relative aspect-[4/3] rounded-xl overflow-hidden bg-muted">
                        {thumbnail ? (
                          <img src={thumbnail} alt={project.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                        ) : (
                      ...
                      <div className="mt-4">
                        ...
                        {project.description && <p className="text-muted-foreground mt-1 line-clamp-2">{project.description}</p>}
                      </div>
                    </motion.div>
                  );
                })}
        ...
        <section className="py-16 px-4 bg-muted/30">
          <div className="container max-w-6xl mx-auto">
                ...
                return (
                  ...
                  >
                    <div className="relative aspect-square rounded-lg overflow-hidden bg-muted">
                      {thumbnail ? (
                        <img src={thumbnail} alt={project.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-muted-foreground">No image</div>
                      )}
            ...
            {projects.length === 0 && (
              <div className="text-center py-12 text-muted-foreground">No projects to display yet.</div>
            )}
          </div>
        </section>
        ...
        <footer className="py-16 px-4 border-t">
          <div className="container max-w-6xl mx-auto text-center">
            <p className="text-muted-foreground">© {new Date().getFullYear()} {designer?.name || 'Designer'}. All rights reserved.</p>
            <p className="text-sm text-muted-foreground mt-2">Portfolio powered by Adorzia Studio</p>
          </div>
        </footer>
      </div>
      ...
      return (
        <section className="py-16 px-4">
          <div className="container max-w-3xl mx-auto">
            ...
            {content.body && <p className="text-lg text-muted-foreground whitespace-pre-wrap">{content.body}</p>}
          </div>
        </section>
      );
    case 'about':
      return (
        <section className="py-16 px-4 bg-muted/30">
          <div className="container max-w-3xl mx-auto">
            ...
            {content.bio && <p className="text-lg text-muted-foreground whitespace-pre-wrap">{content.bio}</p>}
        ...
        <section className="py-16 px-4">
          <div className="container max-w-3xl mx-auto text-center">
            ...
            <div className="flex flex-col items-center gap-4">
              {content.email && (
                <a href={`mailto:${content.email}`} className="flex items-center gap-2 text-lg hover:text-primary transition-colors">
                  <Mail className="w-5 h-5" />
                  {content.email}
                </a>
              )}
              {content.website && (
                <a href={content.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-lg hover:text-primary transition-colors">
