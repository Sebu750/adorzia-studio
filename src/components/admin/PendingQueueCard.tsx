...
export function PendingQueueCard({ items, title, viewAllLink }: PendingQueueCardProps) {
  return (
    <Card className="overflow-hidden bg-admin-card border-admin-border" role="region" aria-labelledby="queue-title">
      <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-admin-border">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-warning/10 flex items-center justify-center">
            <FileSearch className="h-5 w-5 text-warning" />
          </div>
          <div>
            <CardTitle id="queue-title" className="text-lg font-semibold text-admin-foreground">{title}</CardTitle>
            <p className="text-caption text-xs text-admin-muted-foreground">{items.length} items awaiting action</p>
          </div>
        </div>
        <Link to={viewAllLink}>
          <Button variant="outline" size="sm" className="gap-2 btn-press border-admin-border text-admin-foreground hover:bg-admin-foreground hover:text-admin-background hover:border-admin-foreground transition-colors">
            View All
      ...
      <CardContent className="p-0">
        <div className="divide-y divide-admin-border" role="list">
          {items.slice(0, 5).map((item) => (
            <div 
              key={item.id}
              className="flex items-center gap-4 p-4 hover:bg-admin-muted/50 transition-all cursor-pointer group"
              role="listitem"
              tabIndex={0}
            >
              ...
              <div className="flex-1 min-w-0 space-y-1">
                <p className="font-medium truncate text-admin-foreground group-hover:text-admin-foreground/80 transition-colors">
                  {item.title}
                </p>
                <div className="flex items-center gap-2 text-xs sm:text-sm text-admin-muted-foreground flex-wrap">
                  <span className="font-medium text-admin-foreground/70">{item.designer.name}</span>
                  <span className="h-1 w-1 rounded-full bg-admin-border hidden sm:block" />
