...
<<<<<<< HEAD
    ...
    if (profile.founder_tier) {
      if (profile.founder_tier === 'f1') {
        ...
        logStep("Founder bonus applied from profile", { founderTier: profile.founder_tier, founderBonus });
      } else if (profile.founder_tier === 'f2') {
        founderBonus = 5; // +5% for F2 Pioneer
        logStep("Founder bonus applied from profile", { founderTier: profile.founder_tier, founderBonus });
      }
    } else {
      // Fallback: Check if user has a foundation rank purchase (for backward compatibility)
