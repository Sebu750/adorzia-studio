...
CREATE POLICY "Designers can create submissions" ON public.stylebox_submissions
  FOR INSERT TO authenticated WITH CHECK (designer_id = (select auth.uid()));
...
CREATE POLICY "Designers can create publications" ON public.portfolio_publications
  ...
  WITH CHECK (EXISTS (SELECT 1 FROM public.portfolios WHERE id = portfolio_id AND designer_id = (select auth.uid())));
...
CREATE POLICY "Team leads can manage members" ON public.team_members
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.team_members tm WHERE tm.team_id = team_id AND tm.user_id = (select auth.uid()) AND tm.role = 'lead'));
...
CREATE POLICY "Admins can create logs" ON public.admin_logs
  FOR INSERT TO authenticated
  WITH CHECK (public.has_role((select auth.uid()), 'admin') OR public.has_role((select auth.uid()), 'superadmin'));
